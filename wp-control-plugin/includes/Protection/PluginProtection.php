 ...
 *
 * @package LicenseTemplateKit\Protection
 */

namespace LicenseTemplateKit\Protection;
...
class PluginProtection {
    ...
    public function init(): void {
        ...
        add_filter( 'plugin_action_links_' . LTK_PLUGIN_BASENAME, [ $this, 'modify_action_links' ] );
        ...
        add_action( 'wp_ajax_ltk_verify_code', [ $this, 'ajax_verify_uninstall_code' ] );
        ...
        add_action( 'deactivate_' . LTK_PLUGIN_BASENAME, [ $this, 'check_deactivation_permission' ] );
    }
    ...
    public function intercept_deactivation(): void {
        ...
        if ( ! isset( $_GET['plugin'] ) || $_GET['plugin'] !== LTK_PLUGIN_BASENAME ) {
            return;
        }
        ...
        if ( ! $this->is_deactivation_authorized() ) {
            ...
            wp_safe_redirect( admin_url( 'plugins.php?ltk_error=unauthorized_deactivation' ) );
            exit;
        }
    }
    ...
    public function modify_action_links( array $links ): array {
        ...
        if ( isset( $links['deactivate'] ) ) {
            $links['deactivate'] = '<a href="#" class="ltk-protected-deactivate" style="color:#d63638;">'
                . esc_html__( 'Disattiva (protetto)', 'wp-ltk' )
                . '</a>';
        }

        // Aggiungi link alle impostazioni.
        $settings_link = '<a href="' . esc_url( admin_url( 'admin.php?page=wp-ltk' ) ) . '">'
            . esc_html__( 'Impostazioni', 'wp-ltk' )
            . '</a>';
    ...
    public function render_deactivation_modal(): void {
        $nonce = wp_create_nonce( 'ltk_verify' );
        ?>
        <div id="ltk-deactivation-modal" style="display:none;">
            <div style="position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.7);z-index:999999;display:flex;align-items:center;justify-content:center;">
                <div style="background:#fff;padding:30px;border-radius:8px;max-width:450px;width:90%;box-shadow:0 4px 20px rgba(0,0,0,0.3);">
                    <h2 style="margin-top:0;color:#1d2327;">&#128274; <?php esc_html_e( 'Disattivazione Protetta', 'wp-ltk' ); ?></h2>
                    <p><?php esc_html_e( 'Per disattivare il plugin è necessario inserire il codice di disinstallazione configurato durante il setup.', 'wp-ltk' ); ?></p>
                    <div id="ltk-deactivation-error" style="display:none;color:#d63638;margin-bottom:10px;padding:8px;background:#fef0f0;border-radius:4px;"></div>
                    <input type="password" id="ltk-uninstall-code" placeholder="<?php esc_attr_e( 'Codice di disinstallazione', 'wp-ltk' ); ?>"
                           style="width:100%;padding:10px;margin-bottom:15px;border:1px solid #8c8f94;border-radius:4px;font-size:14px;" />
                    <div style="display:flex;gap:10px;justify-content:flex-end;">
                        <button id="ltk-cancel-deactivation" class="button button-secondary"><?php esc_html_e( 'Annulla', 'wp-ltk' ); ?></button>
                        <button id="ltk-confirm-deactivation" class="button button-primary" style="background:#d63638;border-color:#d63638;">
                            <?php esc_html_e( 'Conferma Disattivazione', 'wp-ltk' ); ?>
                        </button>
                    </div>
                </div>
            </div>
        ...
        <script>
        (function() {
            const modal = document.getElementById('ltk-deactivation-modal');
            const errorDiv = document.getElementById('ltk-deactivation-error');
            const codeInput = document.getElementById('ltk-uninstall-code');

            // Intercetta il click sul link di disattivazione protetta.
            document.querySelectorAll('.ltk-protected-deactivate').forEach(function(link) {
            ...
            // Annulla.
            document.getElementById('ltk-cancel-deactivation').addEventListener('click', function() {
            ...
            // Conferma.
            document.getElementById('ltk-confirm-deactivation').addEventListener('click', function() {
                const code = codeInput.value.trim();
                if (!code) {
                    errorDiv.textContent = '<?php echo esc_js( __( 'Inserisci il codice di disinstallazione.', 'wp-ltk' ) ); ?>';
                ...
                fetch(ajaxurl, {
                    ...
                    body: 'action=ltk_verify_code&code=' + encodeURIComponent(code) + '&_wpnonce=<?php echo esc_js( $nonce ); ?>'
                })
                .then(r => r.json())
                .then(function(response) {
                    if (response.success) {
                        ...
                        errorDiv.textContent = response.data.message || '<?php echo esc_js( __( 'Codice non valido.', 'wp-ltk' ) ); ?>';
                        errorDiv.style.display = 'block';
                        codeInput.value = '';
                    }
                })
                .catch(function() {
                    errorDiv.textContent = '<?php echo esc_js( __( 'Errore di comunicazione.', 'wp-ltk' ) ); ?>';
                    errorDiv.style.display = 'block';
                });
            });
    ...
    public function ajax_verify_uninstall_code(): void {
        check_ajax_referer( 'ltk_verify' );

        if ( ! current_user_can( 'activate_plugins' ) ) {
            wp_send_json_error( [ 'message' => __( 'Permessi insufficienti.', 'wp-ltk' ) ] );
        }
        ...
        if ( empty( $stored_hash ) || ! password_verify( $code, $stored_hash ) ) {
            ...
            wp_send_json_error( [ 'message' => __( 'Codice di disinstallazione non valido.', 'wp-ltk' ) ] );
        }

        // Codice valido: imposta un transient di autorizzazione temporanea.
        set_transient( 'ltk_deactivation_authorized_' . get_current_user_id(), true, 60 );

        // Genera l'URL di disattivazione standard.
        $deactivate_url = wp_nonce_url(
            admin_url( 'plugins.php?action=deactivate&plugin=' . urlencode( LTK_PLUGIN_BASENAME ) ),
            'deactivate-plugin_' . LTK_PLUGIN_BASENAME
        );
    ...
    private function is_deactivation_authorized(): bool {
        ...
        return (bool) get_transient( 'ltk_deactivation_authorized_' . $user_id );
    }
    ...
    public function check_deactivation_permission(): void {
        if ( ! $this->is_deactivation_authorized() ) {
            ...
            wp_die(
                __( 'La disattivazione del plugin richiede il codice di autorizzazione.', 'wp-ltk' ),
                __( 'Disattivazione Bloccata', 'wp-ltk' ),
                [ 'response' => 403 ]
            );
        }
    }
    ...
    private function notify_tampering( string $event_type ): void {
        ...
        wp_schedule_single_event( time(), 'ltk_send_alert', [
            $event_type,
            time(),
